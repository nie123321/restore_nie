...
EVALUATOR = Path(r"M:\picture data\cholec80_t\code\cut\endo_enhancement_demo\runs\A_spatial_fusion_on_off_10k_seed100_20260924\code\evaluate_four_metrics.py")
FIELDS = ["sample_id", "video", "lowlight_relpath", "gt_relpath", "output_filename"]
VAL_COUNT = 200


def assert_manifest_unchanged(recorded: str | None, used: str) -> None:
    if not recorded or recorded != used:
        raise ValueError("split_manifest.csv is missing from the checkpoint or changed since training")
...
def main():
    ...
    parser.add_argument("--method-label", default="PriorFusionU3-val")
    ...
    if payload["config"].get("arch_version") != "prior_fusion_u3_v1":
    ...
    assert_manifest_unchanged(payload["config"].get("manifest_sha256"), used_hash)
    ...
    completed = subprocess.run(infer, check=False, env=env)
    ...
    completed = subprocess.run(metrics_cmd, check=False, env=env)
    if completed.returncode != 0:
        raise SystemExit(completed.returncode)
    ...
    summary = {
        ...
        "data_manifest_sha256": used_hash,
        "eval_manifest_sha256": digest(experiment / "manifest.csv"),
        "samples": len(metrics),
        "split": "val",
        "metrics": means,
        "protocol": f"{EVALUATOR} on quantized RGB PNG",
