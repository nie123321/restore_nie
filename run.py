"""Whole-image paired training and checkpoint-only inference for ColorWaveletNet.

Training reads train and optionally val; it never opens test. RGB stays in
encoded [0, 1]. No resize, crop, colour augmentation, or GT mean matching.
Data loading, sampling, LR, RNG, and output protection follow
...
from losses import color_wavelet_losses
from model import ColorWaveletNet
...
def build_model(model_config: dict) -> ColorWaveletNet:
    ...
    return ColorWaveletNet(**payload)
...
def train(args):
    ...
    model = ColorWaveletNet(
        lut_count=args.lut_count, lut_size=args.lut_size, lut_init_seed=args.seed,
    ).to(device)
    ...
    config = {
        ...
        "loss_weights": {
            ...
            "lambda_lut": args.lambda_lut,
        },
        "loss": "L_rgb + lambda_coarse*L_coarse + lambda_ab*L_ab + lambda_lut*L_lut; "
                "L_rgb/L_coarse on unclamped encoded RGB; Lab on clamp(Y,0,1)",
    ...
    if args.resume:
        ...
        keys = ["arch_version", "model", "data_root", "batch_size", "seed", "lr", "weight_decay",
                "amp", "grad_clip", "lr_schedule", "best_metric", "loss_weights"]
    ...
    val_loader = None
    if args.val_every:
        val_data = PairedImages(data, "val")
        val_names = [p.name for p in val_data.low]
    ...
    with (run / "loss.jsonl").open("a", encoding="utf-8") as log:
        for step, (low, target) in enumerate(loader, start=start_step + 1):
            ...
            with torch.autocast(device_type=device.type, enabled=amp, dtype=torch.float16):
                aux = model(low, return_aux=True)
            loss, parts = color_wavelet_losses(
                aux["output"], aux["coarse"], target, model.delta_luts,
                weights["lambda_coarse"], weights["lambda_ab"], weights["lambda_lut"],
            )
...
def main():
    ...
    training.add_argument("--run-dir", type=Path,
                          default=Path(__file__).parent / "runs" / "color_wavelet_30k_psnr_seed100")
    ...
    training.add_argument("--lut-count", type=int, default=4)
    training.add_argument("--lut-size", type=int, default=17)
    training.add_argument("--lambda-coarse", type=float, default=0.2)
    training.add_argument("--lambda-ab", type=float, default=0.05)
    training.add_argument("--lambda-lut", type=float, default=1e-4)
    ...
    if args.command == "train":
        ...
        if min(args.lambda_coarse, args.lambda_ab, args.lambda_lut) < 0:
