"""Whole-image paired training and checkpoint-only inference for Star-A.
...
def build_model(model_config: dict) -> StarA:
    return StarA(
        arch_version=model_config["arch_version"],
        widths=tuple(model_config["widths"]),
        star_locations=tuple(model_config["star_locations"]),
        ffn_expansion_factor=model_config["star_block"]["ffn_expansion_factor"],
...
def train(args):
    ...
    if args.resume:
        ...
        if previous.get("arch_version") != ARCH_VERSION:
            raise ValueError("Checkpoint is not a star_a_v1 model.")
...
def main():
    ...
    training.add_argument("--run-dir", type=Path,
                          default=Path(__file__).parent / "runs" / "star_a_10k_l1_seed100")
