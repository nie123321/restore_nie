...
from endo_prior_a_u3.data import DEFAULT_DATA, PairedImages, StepBatches, inspect_split
from endo_prior_a_u3.model import ARCH_VERSION, PriorAU3, default_config
from endo_prior_a_u3.runtime import (HERE, atomic_save, build_model, check_resume, compute_loss,
...
def train(args):
    ...
    seed_all(args.seed)
    model = PriorAU3(config["model"]).to(device)
...
def main():
    ...
    training.add_argument("--run-dir", type=Path, default=HERE / "runs" / "prior_a_u3_seed100")
    ...
    verification.set_defaults(function=lambda args: __import__("endo_prior_a_u3.verify", fromlist=["verify"]).verify(args))
