"""Whole-image paired training and checkpoint-only inference for Prior-Fusion-U3.
...
from model import PriorFusionU3, default_config
...
PROTECTED = [
    DEFAULT_DATA,
    Path(r"M:\picture data\cholec80_t\code\cut\endo_enhancement_demo"),
    Path(r"M:\picture data\cholec80_t\code\cut\endo_prior_a"),
    Path(r"M:\picture data\cholec80_t\code\cut\endo_deep_a"),
    Path(r"M:\picture data\cholec80_t\code\cut\endo_star_a"),
...
def inspect_split(data: Path) -> dict:
    ...
    if train_ids & val_ids:
        raise ValueError("train and val sample_id values overlap")
...
def build_model(model_config: dict) -> PriorFusionU3:
    # The memory execution option adds no weights; older v1 checkpoints omit it.
    normalized_config = dict(model_config)
    normalized_config.setdefault("activation_checkpointing", True)
    model = PriorFusionU3(normalized_config)
    if model.config() != normalized_config:
        raise ValueError("Checkpoint architecture settings do not rebuild the same model.")
    return model
...
def train(args):
    ...
    if device.type != "cuda":
        raise ValueError("Prior-A v1 training protocol requires CUDA AMP.")
    ...
    model = PriorFusionU3({
        **default_config(),
        "luma_input_mode": args.luma_input_mode,
        "structure_input_mode": args.structure_input_mode,
        "aux_outputs": args.aux_outputs,
    }).to(device)
    ...
    if args.resume:
        ...
        if previous.get("arch_version") != ARCH_VERSION:
            raise ValueError("Checkpoint is not a prior_fusion_u3_v1 model.")
    ...
    with (run / "loss.jsonl").open("a", encoding="utf-8") as log:
        for step, (low, target) in enumerate(loader, start=start_step + 1):
            ...
            with torch.autocast(device_type=device.type, enabled=amp, dtype=torch.float16):
                if model.aux_outputs:
                    packed = model(low, return_aux=True)
                    output = packed["output"]
                    half = torch.nn.functional.interpolate(target, size=packed["pred_half"].shape[-2:], mode="area")
                    quarter = torch.nn.functional.interpolate(target, size=packed["pred_quarter"].shape[-2:], mode="area")
                    loss = (torch.nn.functional.l1_loss(output.float(), target)
                            + 0.25 * torch.nn.functional.l1_loss(packed["pred_half"].float(), half)
                            + 0.125 * torch.nn.functional.l1_loss(packed["pred_quarter"].float(), quarter))
                else:
                    output = model(low)
                    loss = torch.nn.functional.l1_loss(output.float(), target)
...
def main():
    ...
    training.add_argument("--run-dir", type=Path,
                          default=Path(__file__).parent / "runs" / "prior_fusion_u3")
    training.add_argument("--steps", type=int, required=True)
    training.add_argument("--batch-size", type=int, required=True)
    training.add_argument("--luma-input-mode", choices=["real", "zero"], default="real")
    training.add_argument("--structure-input-mode", choices=["real", "zero"], default="real")
    training.add_argument("--aux-outputs", action="store_true")
