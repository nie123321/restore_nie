"""Whole-image paired training and checkpoint-only inference for Prior-A.
...
FIXED_TRAIN_RECIPE = {
    "steps": 10000, "batch_size": 8, "seed": 100, "workers": 0,
    "lr": 2e-4, "lr_schedule": "cosine", "min_lr": 2e-6,
    "weight_decay": 1e-4, "grad_clip": 1.0, "amp": True,
    "val_every": 500, "best_metric": "l1", "save_every": 1000,
    "log_every": 50,
}
PROTECTED = [
    DEFAULT_DATA,
...
def build_model(model_config: dict) -> PriorA:
    model = PriorA(
        width=model_config["width"],
        spectral_mode=model_config["spectral_mode"],
        output_mode=model_config["output_mode"],
        fusion_mode=model_config["fusion_mode"],
    )
    if model.config() != model_config:
        raise ValueError("Checkpoint model settings differ from fixed prior_a_v1 settings.")
    return model
...
def train(args):
    for key, expected in FIXED_TRAIN_RECIPE.items():
        if getattr(args, key) != expected:
            raise ValueError(f"Prior-A v1 requires {key}={expected!r}; got {getattr(args, key)!r}.")
    ...
    if args.resume:
        ...
        if previous.get("arch_version") != ARCH_VERSION:
            raise ValueError("Checkpoint is not a prior_a_v1 model.")
...
def main():
    ...
    training.add_argument("--run-dir", type=Path,
                          default=Path(__file__).parent / "runs" / "prior_a_10k_seed100")
