...
from .model import ARCH_VERSION, PriorAV2
from .priors import rec709
...
def compute_loss(packed, target):
    output, corrected = packed["output"].float(), packed["corrected"].float()
    target = target.float()
    main = F.l1_loss(output, target)
    # Area Down4 uses output dimensions /4, with no clamping or GT-mean shift.
    h, w = target.shape[-2:]
    size = (max(1, h // 4), max(1, w // 4))
    auxiliary = F.l1_loss(F.interpolate(rec709(corrected), size=size, mode="area"),
                          F.interpolate(rec709(target), size=size, mode="area"))
    return main + .1 * auxiliary, main, auxiliary
...
def load_checkpoint(path):
    ...
    if required - state.keys() or state["config"].get("arch_version") != ARCH_VERSION:
        raise ValueError("Not a complete Prior-A v2 checkpoint")
